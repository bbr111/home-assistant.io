...
|--------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------|------------------------------|
| **host**                 | The IP address or hostname of your Volkszaehler server.                                                                                                                                                               | String              | `localhost`                  |
| **port**                 | The port on which the Volkszaehler server is listening.                                                                                                                                                               | Integer             | `80`                         |
| **uuid**                 | The unique identifier (UUID) of your Volkszaehler data point.                                                                                                                                                      | String              | *Required*                   |
| **name**                 | A custom name for the integration.                                                                                                                                                                                 | String              | `Volkszaehler`               |
| **from**                 | *(Optional)* Start time for data collection. Must be specified in milliseconds since epoch or follow [PHP Date and Time Formats](https://www.php.net/manual/en/datetime.formats.php). If a relative value is provided, it refers to `now`. If `to` is not specified, it defaults to `now` minus 24 hours. | String or Integer   | `None`                       |
| **to**                   | *(Optional)* End time for data collection. Must be specified in milliseconds since epoch or follow [PHP Date and Time Formats](https://www.php.net/manual/en/datetime.formats.php). If a relative value is provided, it refers to the `from` timestamp. If `from` is not specified, it defaults to `now`. | String or Integer   | `None`                       |
| **scan_interval**        | The update interval in seconds. Range: `10` to `86400` seconds.                                                                                                                                                        | Integer             | `60`                         |
| **monitored_conditions** | A list of conditions to monitor. Options: `average`, `consumption`, `max`, `min`, `last`.                                                                                                                         | List of Strings     | `["average", "consumption", "max", "min", "last"]` |

**Note**: The `from` and `to` timestamps must be specified in milliseconds since the Unix epoch (January 1, 1970) or follow [PHP Date and Time Formats](https://www.php.net/manual/en/datetime.formats.php). Relative values can be used as follows:

- **from=[timestamp]**
  - **Description**: Start time for the measurement series.
  - **Behavior**:
    - If a relative value is provided, it refers to `now`.
    - If `to` is not specified, it defaults to `now` minus 24 hours.

- **to=[timestamp]**
  - **Description**: End time for the measurement series.
  - **Behavior**:
    - If a relative value is provided, it refers to the `from` timestamp.
    - If `from` is not specified, it defaults to `now`.

**Important**: Dynamic expressions such as `today` or `now` are supported directly within the `from` and `to` fields.

## Options

After setup, you can modify additional options via the Options Flow. This allows you to adjust the `scan_interval` and `monitored_conditions` without reconfiguring the entire integration.
